...
// ── Dashboard hooks ───────────────────────────────────────────────────────────
export const useDashboardOverview = (wsId: string) =>
  useQuery({
    queryKey: ["dashboard-overview", wsId],
    queryFn:  () => import("@/lib/api").then(m => m.dashboardApi.getOverview(wsId)).then(r => r.data),
    enabled:  !!wsId,
    staleTime: 60_000,
    refetchInterval: 120_000,
  });

export const useDashboardHealth = (wsId: string) =>
  useQuery({
    queryKey: ["dashboard-health", wsId],
    queryFn:  () => import("@/lib/api").then(m => m.dashboardApi.getHealth(wsId)).then(r => r.data),
    enabled:  !!wsId,
    staleTime: 120_000,
  });
